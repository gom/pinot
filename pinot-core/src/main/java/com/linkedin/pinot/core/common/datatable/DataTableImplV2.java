...
public class DataTableImplV2 implements DataTable {
  ...
  private Map<String, Map<Integer, String>> _dictionaryMap;
  private byte[] _fixedSizeDataBytes;
  private ByteBuffer _fixedSizeData;
  private byte[] _variableSizeDataBytes;
  private ByteBuffer _variableSizeData;
  ...
  public DataTableImplV2(int numRows, @Nonnull DataSchema dataSchema,
    ...
    _columnOffsets = computeColumnOffsets();
  ...
  private int[] computeColumnOffsets() {
    int[] columnOffsets = new int[_numColumns];
    for (int i = 0; i < _numColumns; i++) {
      columnOffsets[i] = _rowSizeInBytes;
      FieldSpec.DataType dataType = _dataSchema.getColumnType(i);
      switch (dataType) {
        case BOOLEAN:
          _rowSizeInBytes += 1;
          break;
  ...
  private Map<String, Map<Integer, String>> deserializeDictionaryMap(byte[] bytes)
      throws IOException {
    Map<String, Map<Integer, String>> dictionaryMap = new HashMap<>();
    ...
    for (int i = 0; i < numDictionaries; i++) {
      ...
      dataInputStream.read(columnNameBytes);
  ...
  public <T> T getObject(int rowId, int colId) {
    ...
    try {
      return (T) ObjectCustomSerDe.deserialize(bytes, objectType);
    } catch (IOException e) {
  ...
  public String toString() {
    ...
    for (int rowId = 0; rowId < _numRows; rowId++) {
      for (int colId = 0; colId < _numColumns; colId++) {
        FieldSpec.DataType type = _dataSchema.getColumnType(colId);
        switch (type) {
          case BOOLEAN:
